...
Data downloaded from: https://gis.cdc.gov/grasp/nchhstpatlas/tables.html 

CDC; Infectious Diseases, Opioids and Injection Drug Use: https://www.cdc.gov/pwid/opioid-use.html 

  https://www.ncbi.nlm.nih.gov/pubmed/27763996
  * https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5479631/ 
  * https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5479631/table/T2/?report=objectonly 
  
Post story: https://www.washingtonpost.com/national/unraveling-an-hiv-cluster/2019/11/03/66cf4526-f5af-11e9-8cf0-4cc99f74d127_story.html 

CDC; Year 1 geographic priorities: https://www.cdc.gov/endhiv/data.html
...
hiv.data.2 <- 
 select(hiv.data.2, -c(Geography, FIPS, ï..Indicator, Cases, Rate.per.100000))
...
hiv.data.working <- clean_names(hiv.data)

```

```{r}

 hiv.data.working <- hiv.data %>% 
...
# I am going to make a table for just the states that had counties identified as     at-risk. 
  ...
  at_risk_counties <- hiv_and_pills %>%
    filter(state ==c("AL","AK", "AZ", "CA", "CO", "GA", "IL", "IN", "KY", "MN", "MI", "MO", "MT", "NV", "NC", "OH", "OK", "PA", "TX", "UT", "VT", "WV", "KS", "SC", "VA", "TN"))
...
# Q26: What if I compare pills in 2008 to opioid rate in 2012?
  
  ar_pills_2008 <- at_risk_counties %>%
  filter(year=="2008") %>%
  select(pills_per_person, fips, state)

  ar_hiv_2016 <- at_risk_counties %>%
  filter(year == "2016") %>%
  select(hiv_rate_per_100k, fips, state)

  ar_change <- ar_pills_2008 %>%
